...
var _ = Context("with test files", func() {
	...
	Describe("createNmstateRamDisk", func() {
		...
		AfterEach(func() {
			os.Remove(nmstatectlPath)
		})

		It("ram disk created successfully", func() {
			err := createNmstateRamDisk(extractDir, arch, nmstatectlPath, ramDiskPath)
			Expect(err).ToNot(HaveOccurred())

			exists, err := fileExists(filepath.Join(extractDir, ramDiskPath))
			Expect(err).ToNot(HaveOccurred())
			Expect(exists).To(BeTrue())
		})

		It("missing nmstatectl binary", func() {
			err := createNmstateRamDisk(extractDir, arch, "", ramDiskPath)
			Expect(err).To(HaveOccurred())
		...
		It("cpu arch mismatch", func() {
			...
			err := createNmstateRamDisk(extractDir, "some-arch", nmstatectlPath, ramDiskPath)
			Expect(err).ToNot(HaveOccurred())
